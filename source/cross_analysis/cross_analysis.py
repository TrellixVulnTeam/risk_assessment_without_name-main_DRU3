...
def data_summarize(dataframe):
    ...
    with minor:
        ...
        st.write(f"__{top_minor['company name'][0]}__: {top_minor['minor'][0]}")
        st.markdown(f"__{top_minor['company name'][1]}__: {top_minor['minor'][1]}")
    with major:
        ...
        st.write(f"__{top_minor['company name'][0]}__: {top_minor['major'][0]}")
        st.markdown(f"__{top_minor['company name'][1]}__: {top_minor['major'][1]}")
    with critical:
        ...
        if sum(dataframe["critical"]) != 0:
            ...
            top_minor = dataframe[["company name", "critical"]].head().to_dict("list")
            st.write(f"__{top_minor['company name'][0]}__: {top_minor['critical'][0]}")
            st.markdown(f"__{top_minor['company name'][1]}__: {top_minor['critical'][1]}")

def audit_grade_summary(dataframe):
    audit_grade_label = dataframe["audit grade"].value_counts().to_dict()
    st.write('<style>div.row-widget.stRadio > div{flex-direction:row;}</style>', unsafe_allow_html=True)
    audit_grade_graph = go.Figure(data=[go.Pie(labels=list(audit_grade_label.keys()), values=list(audit_grade_label.values()), textinfo='label+value')])
    audit_grade_graph.update_traces(hoverinfo='label+percent', textinfo='label+value', showlegend=False)    
    audit_grade_graph.update_layout(margin=dict(l=10, r=400, t=10, b=50), width=800,height=500)
...
def audit_result_summary(dataframe):
    ...
    st.write('<style>div.row-widget.stRadio > div{flex-direction:row;}</style>', unsafe_allow_html=True)
    audit_result_graph = go.Figure(data=[go.Pie(labels=list(audit_grade_label.keys()), values=list(audit_grade_label.values()), textinfo='label+value')])
    audit_result_graph.update_traces(hoverinfo='label+percent', textinfo='label+value', showlegend=False)    
    audit_result_graph.update_layout(margin=dict(l=10, r=400, t=10, b=50), width=800,height=500)
    st.plotly_chart(audit_result_graph)

def top_best_supplier(dataframe):
    dataframe.sort_values(["minor", "major", "critical", "fundamental"], ascending = [True, True, True, True], inplace = True)
    st.table(dataframe[["company name", "address", "audit grade", "certificate expiry date"]].head())

def top_worst_supplier(dataframe):
    dataframe.sort_values(["minor", "major", "critical", "fundamental"], ascending = [False, False, False, False], inplace = True)
    st.table(dataframe[["company name", "address", "audit grade", "certificate expiry date"]].head())
...
def score_fluctuate(dataframe):
    ...
    with score_fluctuate_column:
        score_fluctuate_graph = go.Figure(data=[go.Pie(labels=["Score remained the same", "Score changed"], values=[score_stay, score_change], textinfo='label+value')])
        score_fluctuate_graph.update_traces(hoverinfo='label+percent', textinfo='label+value', showlegend=False)
        score_fluctuate_graph.update_layout(margin=dict(l=10, r=400, t=10, b=50), width=800,height=500)
        st.plotly_chart(score_fluctuate_graph)

    with score_increase_column:
        score_fluctuate_graph = go.Figure(data=[go.Pie(labels=["Score Increased", "Score Decreased"], values=[score_increase, score_decrease], textinfo='label+value')])
        score_fluctuate_graph.update_traces(hoverinfo='label+percent', textinfo='label+value', showlegend=False)
        score_fluctuate_graph.update_layout(margin=dict(l=10, r=400, t=10, b=50), width=800,height=500)
        st.plotly_chart(score_fluctuate_graph)
